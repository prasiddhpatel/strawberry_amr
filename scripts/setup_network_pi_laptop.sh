#!/usr/bin/env bash
# WiFi-based ROS 2 network setup for the Pi(4GB)+Laptop topology -- run
# ONCE on each machine with the matching role.
#
# CORRECTED from an earlier revision of this script, which assumed a
# physical Ethernet cable between the Pi and the laptop. That was a real
# design mistake, not a minor detail: the laptop is not mounted on the
...
# a cable to a laptop that isn't on it makes that impossible. This should
# have been reconsidered when the Pi+laptop split was first built, not
# after the fact.
#
# THE DESIGN: the Pi hosts its own WiFi Access Point (hostapd + dnsmasq),
# and the laptop joins it as a client. This is deliberately NOT "both
...
# still run on the laptop (see docs/LAPTOP_PI_SPLIT_ARCHITECTURE.md) --
...
#
# Usage:
#   ./setup_network_pi_laptop.sh pi        (on the Raspberry Pi 4B 4GB)
#   ./setup_network_pi_laptop.sh laptop    (on the laptop)
#
...
# this script's own laptop-role comments below, and
# docs/LAPTOP_PI_SPLIT_ARCHITECTURE.md's network section). hostapd+dnsmasq
...
ROLE="$1"
if [ "$ROLE" != "pi" ] && [ "$ROLE" != "laptop" ]; then
  echo "Usage: $0 [pi|laptop]"
  echo "  pi      -- run this on the Raspberry Pi 4B 4GB (hosts the WiFi AP)"
  echo "  laptop  -- run this on the laptop (joins the Pi's AP)"
  exit 1
fi

SSID="StrawberryAMR"
PI_IP="192.168.30.1"
LAPTOP_IP="192.168.30.100"

if [ "$ROLE" = "pi" ]; then
  echo ">>> Configuring role=pi (WiFi AP host)  this-host=$PI_IP  peer=$LAPTOP_IP"
  echo ""
...
HOSTAPD
  ...
  # convenience (e.g. a phone for quick monitoring) -- the laptop itself
  # does NOT rely on this; it pins its own static IP directly (see the
  # laptop role below), same pattern this script already used for the
...
DNSMASQ
  ...
  echo ">>> Configuring role=laptop (WiFi AP client)  this-host=$LAPTOP_IP  peer=$PI_IP"
  echo ""
  ...
  sudo nmcli connection modify "$CONN_NAME" \
    ipv4.addresses "${LAPTOP_IP}/24" ipv4.method manual ipv6.method disabled \
    connection.autoconnect yes
  sudo nmcli connection up "$CONN_NAME"

  IFACE=$(nmcli -t -f DEVICE,CONNECTION dev status | awk -F: -v c="$CONN_NAME" '$2==c{print $1}')
  echo ">>> Joined '$SSID', pinned to $LAPTOP_IP/24 on $IFACE (persistent,"
...
BASHRC_MARK="# --- strawberry_amr_ws pi-laptop network setup (added by setup_network_pi_laptop.sh) ---"
...
echo "      ping $PI_IP        # from the laptop"
echo "      ping $LAPTOP_IP    # from the Pi"
...
echo "=== the laptop is positioned, that is a real, physical limitation to"
echo "=== plan around (e.g. keep the laptop within the tunnel, not outside"
